# SDE Visualization (Modular)

Quick-start, production-ready demo to explore diffusion/SDE trajectories with interactive controls.

## Run it
- Easiest: open `index.html` in your browser.
- Or local server (optional):
  - `python -m http.server 8000` (then visit `http://localhost:8000/demos/sde-modular/`)

## What you get
- Interactive controls: paths count, steps, SDE type (VP/VE/Sub-VP), initial distribution, color scheme, background, legend/ODE/SDE/heatmap toggles, regenerate/reset.
- 5-panel Plotly view: data → forward SDE → prior → reverse SDE → data.
- Responsive, zero-build, CDN Plotly.

## Common presets
- Teaching: paths=20, colors=rainbow, background=white, toggle ODE/SDE on.
- Publication: paths≈20, steps≈300, colors=monochrome, background=academic, legend off.
- Performance: paths=5-10, steps=50-100, heatmap off.

## Docs
All detailed docs moved to `docs/` (full README, quickstart, features, comparisons, structure).
